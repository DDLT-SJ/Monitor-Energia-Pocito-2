/*
 * ============================================================
 *  Monitor Energía Pocito — ESP32-C3        VERSION FINAL
 * ============================================================
...
// ─────────────────────────────────────────────
//  ADC — LECTURA DE BATERIA LITIO (GPIO2)
// ─────────────────────────────────────────────
#define PIN_BATERIA 2
#define ADC_ATTENUATION ADC_11db
#define ADC_RESOLUTION 12
#define VOLTAJE_DIVISOR 2.0
...
const unsigned long intervaloBateria = 60000;
...
void leerYPublicar() {
  ...
  unsigned long nowBateria = millis();
  if (nowBateria - ultimaLecturaBateria >= intervaloBateria) {
    int lectura = analogRead(PIN_BATERIA);
    voltajeBateria = (lectura / (float)MAX_ADC) * VOLTAJE_REFERENCIA * VOLTAJE_DIVISOR;
    Serial.printf("Bateria: %.2fV (ADC: %d)\n", voltajeBateria, lectura);

    String jsonBateria = "{\"bateria_voltaje\":" + String(voltajeBateria, 2) + "}";
    mqttClient.publish((String(MQTT_TOPIC) + "/bateria").c_str(), jsonBateria.c_str());

    ultimaLecturaBateria = nowBateria;
  }
}
